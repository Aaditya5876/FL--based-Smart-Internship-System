...
def main():
	...
	ap.add_argument("--outdir", type=str, default="src/research/RQ3/results/fedprox")
	args = ap.parse_args()
	...
	for seed in args.seeds:
			...
			for cid, grp in base_train_df.groupby("client_id"):
				...
				for _ in range(args.local_epochs):
					for xb, yb in tr_loader:
						...
						prox = 0.0
						for (n1, p1), (n2, p2) in zip(local.named_parameters(), global_model.named_parameters()):
							prox = prox + torch.sum((p1 - p2.detach()) ** 2)
						loss = loss + 0.5 * args.mu * prox
						loss.backward()
						opt.step()
		...
		for k in args.k_list:
			...
			all_logs.append({"seed": seed, "k": k, "scenario": args.scenario, "held_client": args.held_client, "mu": args.mu, "metrics": metrics})

	out_path = os.path.join(args.outdir, f"fedprox_{args.scenario}.json")
